package uz.uzinfocom.app.platform.reference.application.icd10.dto;
...
@Schema(description = """
        Данные для создания узла классификатора МКБ-10. Идентификаторы назначаются внешним источником
        (МКБ-10 ВОЗ), а не генерируются приложением, поэтому id обязателен и должен соответствовать
        нумерации источника, чтобы связи родитель-потомок оставались корректными при повторном импорте.
        """)
public record Icd10CreateRequest(
        @Schema(description = "Внешний идентификатор классификатора, присвоенный исходным набором данных МКБ-10 ВОЗ.",
                example = "1500", requiredMode = Schema.RequiredMode.REQUIRED)
        @NotNull(message = "{reference.icd10.id.required}")
        @Positive(message = "{reference.icd10.id.positive}")
        Long id,
        ...
        @Positive(message = "{reference.icd10.parent_id.positive}")
        Long parentId,

        @Schema(description = "Уникальный код МКБ-10.", example = "A15", requiredMode = Schema.RequiredMode.REQUIRED)
        @NotBlank(message = "{reference.icd10.code.required}")
        @Size(max = 20, message = "{reference.icd10.code.max_length}")
        String code,

        @Schema(description = "Глубина данного узла в иерархии классификатора, начиная с 0.", example = "3")
        @NotNull(message = "{reference.icd10.level.required}")
        @PositiveOrZero(message = "{reference.icd10.level.positive_or_zero}")
        Integer level,

        @Schema(description = "Признак того, что узел является конечным (назначаемым кодом диагноза), а не заголовком категории.",
                example = "true")
        @NotNull(message = "{reference.icd10.last_level.required}")
        Boolean lastLevel,
        ...
        @Size(max = 2000, message = "{reference.icd10.comment.max_length}")
        String comment,

        @Schema(description = "Сколько раз может быть использован/выбран данный код. По умолчанию 1, если не указано.",
                example = "1")
        @Min(value = 0, message = "{reference.icd10.usage_limit.min}")
        Integer usageLimit
) {
}
