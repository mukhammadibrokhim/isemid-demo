...
/**
 * ICD-10 classifier node, bulk-imported from an external source (WHO
...
@Table(
        name = "ref_icd10",
        indexes = {
                @Index(name = "idx_ref_icd10_parent_id", columnList = "parent_id"),
                @Index(name = "idx_ref_icd10_code", columnList = "code"),
                @Index(name = "idx_ref_icd10_deleted", columnList = "deleted")
        },
        uniqueConstraints = {
                @UniqueConstraint(name = "uk_ref_icd10_code", columnNames = "code")
        }
)
@SuperBuilder(toBuilder = true)
@NoArgsConstructor
@AllArgsConstructor
public class Icd10 extends ExternallyIdentifiedEntity {
    ...
    @Builder.Default
    private List<Icd10> children = new ArrayList<>();
