...
public class LoginController {
    ...
    @Operation(
            ...
            description = "Если у провайдера (path-параметр {provider}) настроен revoke-url/logout-url, "
                    + "лучшим образом отзывает переданные access/refresh-токены и там же, а затем в любом "
                    + "случае заносит их в локальный чёрный список - проверяется на каждом "
                    + "аутентифицированном запросе и в /refresh. Чёрный список - это то, что реально "
                    + "прекращает действие токена на этом бэкенде: и SSO, и DHP валидируются локально по "
                    + "подписи, поэтому один только вызов provider logout/revoke не помешал бы уже "
                    + "выданному токену остаться рабочим здесь до истечения его срока - именно поэтому "
                    + "провал upstream-вызова не блокирует занесение в чёрный список."
    )
