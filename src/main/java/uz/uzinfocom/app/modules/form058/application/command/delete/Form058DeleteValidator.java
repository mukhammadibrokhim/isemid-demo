...
public class Form058DeleteValidator {
    ...
    public void validate(Form058 form058, boolean linkedCardsExist) {
        ...
        if (!form058.getStatus().physicallyDeletable()) {
            throw new uz.uzinfocom.app.features.form058.domain.exception.InvalidForm058StateException("error.form058.delete-not-allowed", form058.getStatus());
        }

        if (linkedCardsExist) {
            throw new uz.uzinfocom.app.features.form058.domain.exception.InvalidForm058StateException("error.form058.cards-linked");
        }
    }
}
