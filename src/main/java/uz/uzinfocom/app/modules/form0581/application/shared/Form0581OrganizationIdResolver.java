...
public class Form0581OrganizationIdResolver {
    ...
    public Long resolveActiveId(UUID uuid) {
        if (uuid == null) {
            throw new Form0581ValidationException("validation.form0581.organization.required");
        }
        return organizationRepository.findActiveIdByUuid(uuid)
                .orElseThrow(() -> new Form0581ValidationException("error.organization.not-found", uuid));
    }
    ...
    public OrganizationLocalizedName resolveActiveNameFields(Long id) {
        ...
        return organizationRepository.findActiveNameFieldsById(id)
                .orElseThrow(() -> new Form0581ValidationException("error.organization.not-found", id));
    }
}
