...
public class Form0581StatsRepository extends AbstractCaseStatsRepository<Form0581> {
    ...
    private static final List<Form0581Status> PENDING_STATUSES = Arrays.stream(Form0581Status.values())
            .filter(Form0581Status::isPending)
            .toList();
    ...
    /**
     * Count of cases that have not yet reached a final outcome ({@link
     * Form0581Status#isPending()}) — a direct {@code COUNT(*)}, not a
     * Java-side filter+sum over {@link #countByStatus}.
     */
    ...
    public List<Form0581Icd10CountResponse> topIcd10(ResolvedOrganizationScope scope, Boolean received, int limit) {
        return topGrouped(
                this::icd10Code,
                (root, cb) -> scopePredicateFactory.applyDirectionScope(root, cb, scope, received),
                limit,
                Form0581Icd10CountResponse::new
        );
    }
     ...
     * authorization check themselves.
     */
    public List<Form0581Icd10CountResponse> topIcd10Unscoped(int limit) {
        return topGrouped(
                this::icd10Code,
                null,
                limit,
    ...
    private Path<String> icd10Code(Root<Form0581> root, CriteriaBuilder cb) {
