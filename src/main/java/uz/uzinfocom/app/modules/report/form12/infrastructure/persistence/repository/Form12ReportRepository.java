 ...
 * <p>
 * The "diagnosis" of a case is {@code coalesce(f.final_icd10_code,
 * f.icd10_code)} — the confirmed code, falling back to the initial one when no
 * separate final code was recorded. Matching on that single value (rather than
 * "initial OR final is in the set", as {@code Form1ReportRepository}'s
 * CONFIRMED block does) keeps a case from landing in two different
 * nosological-form rows at once.
 * <p>
