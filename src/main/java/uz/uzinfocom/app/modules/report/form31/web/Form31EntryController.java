...
import uz.uzinfocom.app.modules.report.form31.application.export.Form31EntryExcelExportSource;
...
public class Form31EntryController {
    ...
    private final Form31EntryExcelExportSource form31EntryExcelExportSource;
    ...
    @Operation(
            summary = "Экспорт записей Shakl №3-1 в Excel",
    ...
    @PostMapping(ApiPaths.Form31Entry.EXPORT)
    @PreAuthorize("isAuthenticated() and hasAuthority('PERMISSION_REPORTS_READ')")
    public ApiResponse<ExportJobResponse> export(
            @ParameterObject @Valid Form31EntryFilterRequest filter
    ) {
        return ApiResponse.success(
                messageResolver.resolve("export.job.submitted"),
                exportJobService.submit(form31EntryExcelExportSource, filter)
        );
    }
}
