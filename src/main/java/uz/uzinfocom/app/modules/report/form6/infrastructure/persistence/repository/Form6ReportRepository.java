...
public class Form6ReportRepository {
    ...
    public Form6AgeBreakdownProjection countAgeBreakdown(
        ...
        return rows.isEmpty() ? Form6AgeBreakdownProjection.EMPTY : toAgeBreakdown((Object[]) rows.getFirst(), 0);
    }

    /**
     * Same age buckets as {@link #countAgeBreakdown}, but one row per organization id instead
     * of one aggregate over the whole list — for a report's organization-level (leaf) export
     * rows, which have no region/district code to resolve a subtree from and so cannot use
     * {@link #countAgeBreakdown} one node at a time without an N+1 query per organization. An
     * organization with zero matching cases in the range is simply absent from the result (see
     * {@code countGroupedByOrganization}).
     */
    public List<Form6AgeBreakdownByOrganizationProjection> countAgeBreakdownGroupedByOrganization(
            List<Long> organizationIds,
            Instant fromInclusive,
            Instant toExclusive,
            String diagnosisCode
        ...
        String sql = "select t.sender_organization_id as organization_id, " + AGE_BREAKDOWN_COLUMNS
                + " from (" + unionSource(organizationIds) + ") t group by t.sender_organization_id";
        ...
        return rows.stream()
                .map(row -> {
                    Object[] r = (Object[]) row;
                    return new Form6AgeBreakdownByOrganizationProjection(
                            ((Number) r[0]).longValue(), toAgeBreakdown(r, 1)
                    );
                })
                .toList();
    }
    ...
    private Form6AgeBreakdownProjection toAgeBreakdown(Object[] row, int offset) {
