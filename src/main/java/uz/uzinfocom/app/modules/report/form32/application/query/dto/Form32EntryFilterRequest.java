...
@Schema(description = "Параметры фильтрации и пагинации таблицы ручных записей Shakl №3-2.")
public record Form32EntryFilterRequest(
        @Schema(description = "Номер страницы, начиная с 1.", example = "1")
        @Min(value = 1, message = "{report.form32_entry.filter.page.min}")
        Integer page,

        @Schema(description = "Количество записей на странице. Максимальное значение — 200.", example = "20")
        @Min(value = 1, message = "{report.form32_entry.filter.size.min}")
        @Max(value = 200, message = "{report.form32_entry.filter.size.max}")
        Integer size,
