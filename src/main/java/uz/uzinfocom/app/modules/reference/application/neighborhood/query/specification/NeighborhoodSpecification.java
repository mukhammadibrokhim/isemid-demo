package uz.uzinfocom.app.modules.reference.application.neighborhood.query.specification;
...
public final class NeighborhoodSpecification {

    private NeighborhoodSpecification() {
    }

    public static Specification<Neighborhood> byFilter(NeighborhoodFilterRequest request) {
        return (root, query, cb) -> {
            ...
            if (StringUtils.hasText(request.tin())) {
                predicates.add(cb.equal(
                        cb.upper(root.get("tin")),
                        request.tin().trim().toUpperCase(Locale.ROOT)
                ));
            }
