package uz.uzinfocom.app.modules.reference.application.icd10.query.dto;
...
public record Icd10FilterRequest(
        @Schema(description = "Номер страницы, начиная с 1.", example = "1")
        @Min(value = 1, message = "{reference.icd10.filter.page.min}")
        Integer page,

        @Schema(description = "Количество записей на странице. Максимальное значение — 200.", example = "20")
        @Min(value = 1, message = "{reference.icd10.filter.size.min}")
        @Max(value = 200, message = "{reference.icd10.filter.size.max}")
        Integer size,
