package uz.uzinfocom.app.modules.iam.application.action.query.dto;
...
@Schema(description = "Параметры фильтрации и пагинации списка действий.")
public record ActionFilterRequest(
        @Schema(description = "Номер страницы, начиная с 1.", example = "1")
        @Min(value = 1, message = "{action.filter.page.min}")
        Integer page,

        @Schema(description = "Количество записей на странице.", example = "20")
        @Min(value = 1, message = "{action.filter.size.min}")
        @Max(value = 200, message = "{action.filter.size.max}")
        Integer size,
        @Schema(description = "Поле для сортировки.", example = "code")
        String sortBy,
        @Schema(description = "Направление сортировки.", example = "asc", allowableValues = {"asc", "desc"})
        String sortDir,

        @Schema(description = "Фильтр по коду действия.", example = "READ")
        String code,
