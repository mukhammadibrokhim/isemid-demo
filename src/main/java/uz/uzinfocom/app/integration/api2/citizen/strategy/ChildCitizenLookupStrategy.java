...
public class ChildCitizenLookupStrategy implements CitizenLookupStrategy {

    private final CitizenApi2Client citizenApi2Client;

    @Override
    public CitizenLookupType type() {
        return CitizenLookupType.CZ;
    }

    @Override
    public CitizenLookupResult lookup(CitizenLookupRequest request) {
        return citizenApi2Client.lookupChild(request.series(), request.number());
    }
}
