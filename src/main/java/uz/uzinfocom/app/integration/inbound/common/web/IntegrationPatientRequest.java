...
/**
 * Integration-only equivalent of {@code PatientRequest} - identical patient
 * fields, but its affiliations use {@link IntegrationPatientAffiliationRequest}
 * (organizationId + organizationUuid + organizationName) instead of the
 * regular request's organizationId-only shape, since integration callers
 * (DMED and others) may reference organizations not yet registered here.
 */
@Schema(description = "Сведения о пациенте, регистрируемом вместе с формой - интеграционный формат.")
public record IntegrationPatientRequest(
        ...
        @Valid
        List<IntegrationPatientAffiliationRequest> affiliations

) {

    public IntegrationPatientRequest {
