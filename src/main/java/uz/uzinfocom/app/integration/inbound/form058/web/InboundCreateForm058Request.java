...
/**
 * Inbound-integration equivalent of {@code CreateForm058Request} — same
 ...
 * the {@code Form058} entity's own embedded structure ({@code diagnosisInfo},
 * {@code clinicalInfo}, {@code dateInfo}, {@code epidemicInfo},
 * {@code reportInfo}) rather than the frontend's flat shape, since integration
 * callers submit against the domain model directly, not against a
 * screen-shaped form. See {@code InboundForm058Validator} for the additional
 * cross-field checks applied on top of these annotations.
 */
@Schema(description = "Запрос на создание формы №058 через интеграционный API.")
public record InboundCreateForm058Request(
