...
| `report` | Cross-form, organization-hierarchy statistical reports (Forms 1, 2, 3-1, 3-2, 4, 6, 7, 8, 9, 10, 11, 12, 13) built on `form058`/`form0581` data plus hand-entered counts that have no other source of truth. Every drill-down report shares one hierarchy pattern (republic → region → district → organization, one level per call) via `report.shared.ReportHierarchyService`. Form 12 is disease-first: its rows are the manual-report catalog entries tagged `FORM_12`, geography is the drill-down. Form 13 is the transpose of Form 12: geography-first rows, one column per manual-report catalog entry tagged `FORM_13`. | `/v1/reports/**` | — |
