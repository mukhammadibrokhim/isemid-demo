...
| `report` | Cross-form, organization-hierarchy statistical reports (Forms 1, 2, 3-1, 3-2, 4, 6, 7, 8, 9, 11, 12) built on `form058`/`form0581` data plus hand-entered counts that have no other source of truth. Every drill-down report shares one hierarchy pattern (republic → region → district → organization, one level per call) via `report.shared.ReportHierarchyService`. Form 12 is disease-first: its rows are the manual-report catalog entries tagged `FORM_12`, geography is the drill-down. | `/v1/reports/**` | — |
