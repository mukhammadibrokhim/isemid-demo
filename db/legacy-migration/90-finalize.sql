...
-- MUHIM: jadval id ustunlari Liquibase `autoIncrement` -> Postgres'da
-- `GENERATED BY DEFAULT AS IDENTITY`. Bunday sequence bilan ustun orasidagi
-- pg_depend bog'lanishi `deptype = 'i'` (internal), `'a'` (auto — faqat
-- `serial`/`DEFAULT nextval`) EMAS. Shu sabab avvalgi `deptype='a'` filtri
-- barcha biznes jadvallar sequence'ini jimgina o'tkazib yuborardi va
-- `00-prep` `RESTART IDENTITY` dan keyin ular 1 da qolib, ilova birinchi
-- INSERT'da `duplicate key ... _pkey` (masalan `pt_address_pkey`) berardi.
-- pg_get_serial_sequence() identity ustunlar uchun ham to'g'ri ishlaydi.
DO $$
DECLARE r record;
BEGIN
  FOR r IN
  ...
  LOOP
    EXECUTE format(
      ...
      || ' GREATEST((SELECT COALESCE(max(%I),0) FROM public2.%I), 1),'
      || ' (SELECT count(*) > 0 FROM public2.%I))',
      'public2.'||r.tbl, r.col, r.col, r.tbl, r.tbl);
  END LOOP;
END $$;
