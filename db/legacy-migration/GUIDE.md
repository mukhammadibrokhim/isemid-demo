...
| —  | `96-fix-organization-provider-key.sql` | **faqat kerak bo'lganda** — `10-organization.sql` `provider_key` backfill'idan OLDIN ishga tushirilgan bo'lsa (eski migratsiya natijasi). Xavfsiz, mustaqil, `00-prep` kerak emas, istalgan marta qayta ishga tushса bo'ladi. |
...
| `IllegalArgumentException: providerKey must be specified...` admin tashkilotni qayta sinxronlaganda (`syncByUuid`) | Migratsiya qilingan tashkilotda `provider_key` NULL (eski `10-organization.sql` bilan ko'chirilgan, `provider_key` backfill'dan oldin). Yechim: `96-fix-organization-provider-key.sql` ni yugurting. |
